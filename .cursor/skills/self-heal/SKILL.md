...
name: self-heal
description: >-
  Repairs drifted Playwright locators after triage classifies a red run as a
  test issue (selector drift, not an app bug). Use when the build is red because
  a locator broke, fix the drifted selector, the test broke after a UI change,
  or heal the suite. Use ONLY after ci-failure-triage classifies the failure as
  drift — never for a real app bug; route those to jira-bug-reporter instead.
---
...
2. If classification is **real app bug**, missing, or ambiguous → **stop**.
   Route to `jira-bug-reporter`. Do not heal.
...
Cross-reference `pages/` per `pom-conventions`. The patch target is always the
POM, never the spec.

### 2. Re-discover the element (Playwright MCP a11y tree)

Use the Playwright MCP (`user-Playwright`) against the live app
(`DIDAXIS_URL` from `.env`):
...
4. Derive the **new locator** using the project hierarchy:
...
- Keep role-based locators; change `name`, `exact`, or scoping
  (`dialog.getByRole`) as needed.
...
```bash
npm run test:ci -- tests/<spec>.spec.ts --project=chromium
```
...
- **Same locator error** → reconsider the a11y discovery; do not weaken
  assertions.
- **Different failure / app behavior wrong** → stop healing; re-triage. Likely
  real app bug → `jira-bug-reporter`.
- **Green only after touching assertions** → **escalate**; revert assertion
  edits.
...
**POM:** `pages/<file>.ts` → `<propertyName>`
...
### Verification
- `npm run test:ci -- tests/<spec>.spec.ts --project=chromium` → **green**
...
- `ci-failure-triage` — must run first; supplies classification
- `pom-conventions` — locator hierarchy and file layout
- `jira-bug-reporter` — when triage says real app bug
