...
test.describe("DS-1 — Create new academic program", () => {
  ...
  test.describe("Positive flows", () => {
    ...
    test("TC-002 — Program is created successfully with valid inputs", async ({
      ...
      const programName = `DS1 Web Development ${Date.now()}`;
    ...
    test("TC-004 — Create a program with an empty description", async ({ page }) => {
      const programName = `DS1 Cloud Engineering ${Date.now()}`;
      const programs = new ProgramsPage(page);
  ...
  test.describe("Negative flows", () => {
    ...
    test("TC-006 — Program is not created without submitting the form", async ({
      ...
      const programName = `DS1 NoSubmit ${Date.now()}`;
      const description = "Full-stack web development program";
      const programs = new ProgramsPage(page);
    ...
    test("TC-007 — Closing the modal without saving does not create a program", async ({
      ...
      const programName = `DS1 Cancel ${Date.now()}`;
    ...
    test("TC-007b — Close X without saving does not create a program", async ({ page }) => {
      const programName = `DS1 CloseX ${Date.now()}`;
      const description = "Machine learning fundamentals";
      const programs = new ProgramsPage(page);
    ...
    test("TC-007c — Click outside modal without saving does not create a program", async ({
      ...
      const programName = `DS1 Outside ${Date.now()}`;
  ...
  test.describe("Edge cases", () => {
    test("TC-008 — Program name with leading and trailing whitespace is handled on submit", async ({
      ...
      const trimmedName = `DS1 Trim ${Date.now()}`;
    ...
    test("TC-010 — Program name with special characters is accepted", async ({
      ...
      const programName = `DS1 Informatique & IA - Niveau ${Date.now()}`;
    ...
    test("TC-011 — Rapid double-click on Create creates only one program", async ({
      ...
      test.fail(true, "Known app bug — double-click Create submits duplicate programs.");
      const programName = `DS1 Mobile ${Date.now()}`;
    ...
    test("TC-012 — Program list updates immediately after successful creation", async ({
      ...
      const programName = `DS1 Cybersecurity ${Date.now()}`;
    ...
    test("TC-021 — Modal dismisses promptly after successful Create", async ({ page }) => {
      const programName = `DS1 ModalTiming ${Date.now()}`;
